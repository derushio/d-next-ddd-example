...
graph TB
    subgraph "✅ DDDアプローチ"
        A[UserDomainService] --> B[canChangeEmail]
        B --> C[email !== newEmail]
        B --> D[accountStatus === 'active']
        B --> E[email not duplicate]
        C --> F{変更可能？}
        D --> F
        E --> F
        F -->|Yes| G[UpdatedUser作成]
        F -->|No| H[DomainError]
        G --> I[email = newEmail]
        G --> J[updatedAt設定]
    end
...
graph TB
    subgraph "User Entity（Aggregate Root）"
        ...
        A --> C[email: Email]
        A --> D[name: string]
        A --> E[passwordHash: string]
        A --> F[createdAt: Date]
        A --> G[updatedAt: Date]
    end

    subgraph "ビジネスルール（Immutable）"
        H[changeEmail] --> I{email !== newEmail?}
        I -->|Yes| J[新Userインスタンス返却]
        I -->|No| K[DomainError]

        L[updateProfile] --> M{name valid?}
        M -->|Yes| N[新Userインスタンス返却]
        M -->|No| O[DomainError]
    end
    ...
    style N fill:#065f46,stroke:#10b981,stroke-width:2px,color:#ffffff
...
graph TB
    subgraph "UserDomainService"
        A[canChangeEmail] --> B{email !== newEmail?}
        B -->|No| C[false]
        B -->|Yes| D[userRepository.findByEmail]
        D --> E{existing user?}
        E -->|Yes| F[false]
        E -->|No| G[true]

        H[validateEmailUniqueness] --> I{existing user?}
        I -->|Yes| J[DomainError]
        I -->|No| K[OK]

        N[validateUserData] --> O{name.length >= 1?}
        O -->|No| P[DomainError]
...
graph TB
    ...
    subgraph "✅ DDDアプローチ"
        B1[ChangeEmailUseCase] --> B2[UserRepository.findById]
        B2 --> B3[UserDomainService.canChangeEmail]
        B3 --> B4{変更可能？}
        B4 -->|Yes| B5[User.changeEmail]
...
```typescript
// メール変更条件の変更例：「VIPメンバーのみメール変更可能」
export class UserDomainService {
 async canChangeEmail(
  user: User,
  newEmail: Email,
