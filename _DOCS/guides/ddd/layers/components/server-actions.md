...
export async function loginAction(
 ...
 try {
  ...
  // セッションCookie設定
  (await cookies()).set('session-token', result.value.sessionToken, {
   httpOnly: true,
...
export async function createUserAction(formData: FormData) {
 ...
 const isValidEmail = /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email); // 禁止

 // パスワードハッシュ化ロジック（Infrastructure/Application Layerの責務）
 const passwordHash = sha256(formData.get('password') as string); // 禁止
...
describe('createUserAction', () => {
 ...
 it('UseCase失敗時にエラーメッセージを返す', async () => {
  ...
  mockCreateUserUseCase.execute.mockResolvedValue(
   err({ message: 'このメールアドレスは既に登録されています', code: 'EMAIL_ALREADY_EXISTS' }),
  );
