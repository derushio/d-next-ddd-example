...
3. **ビジネスルール・ドメインロジックの実装**
   ...
   const validateEmailFormat = (email: string) => {
    return /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email); // Domain Layerの責務（Email Value Object）
   };
   ```
...
```typescript
// ✅ 良い例：単一のユースケースに集中
// ※ 実際の戻り値は必ず Result<CreateUserResponse, AppError> を使用すること（概念説明のため省略）
...
```typescript
// ✅ 推薦：レイヤー固有のエラー処理
// ※ 本プロジェクトでは例外スロー禁止。実際には Result<T, AppError> を返すパターンを使用すること。
//    詳細は neverthrow-patterns スキルを参照。
export class CreateUserUseCase {
 async execute(request: CreateUserRequest): Promise<CreateUserResponse> {
  try {
   // ドメインロジック実行
