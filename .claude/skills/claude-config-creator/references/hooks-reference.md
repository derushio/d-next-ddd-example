    ...
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "pnpm biome format --write \"$CLAUDE_FILE_PATH\" 2>/dev/null || true",
            "timeout": 30
          }
        ]
      }
...
Prettierを使用するプロジェクトの場合は以下のコマンドに変更:
    ...
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "pnpm biome format --write \"$CLAUDE_FILE_PATH\"",
            "timeout": 30
          },
          {
            "type": "command",
            "command": "pnpm biome lint --write \"$CLAUDE_FILE_PATH\"",
            "timeout": 30
          }
        ]
      }
