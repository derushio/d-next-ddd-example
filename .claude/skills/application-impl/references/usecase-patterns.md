...
export class CompleteUserRegistrationUseCase {
  ...
  async execute(
  ...
  ): Promise<Result<CompleteRegistrationResponse, AppError>> {
    ...
    try {
      ...
      return err({ message: '登録完了処理に失敗しました', code: 'UNEXPECTED_ERROR' });
    }
  }
}
```
...
export class GetUsersUseCase {
  ...
  async execute(
  ...
  ): Promise<Result<GetUsersResponse, AppError>> {
    ...
    try {
      ...
      if (request.pageSize < 1 || request.pageSize > 100) {
        return err({
          message: 'ページサイズは1-100の範囲である必要があります',
      ...
      return err({ message: 'ユーザー一覧取得に失敗しました', code: 'UNEXPECTED_ERROR' });
    }
  }
}
```
...
export class BulkUpdateUsersUseCase {
  ...
  async execute(
  ...
  ): Promise<Result<BulkUpdateUsersResponse, AppError>> {
    ...
    try {
      ...
      if (request.userIds.length === 0) {
        return err({ message: '更新対象のユーザーIDが指定されていません', code: 'NO_USERS' });
      }

      if (request.userIds.length > 1000) {
        return err({
          message: '一度に更新できるユーザー数は1000件までです',
          code: 'TOO_MANY_USERS',
        });
      }
      ...
      return err({ message: '一括更新に失敗しました', code: 'UNEXPECTED_ERROR' });
    }
  }
...
export class SignInUseCase {
  ...
  ): Promise<Result<SignInResponse, AppError>> {
    ...
    try {
      ...
      if (request.ipAddress) {
        ...
        if (!rateLimitResult.allowed) {
          ...
          return err({
            message: `リクエスト数が上限に達しました。${retryAfterSeconds}秒後に再試行してください。`,
      ...
      return err({ message: 'サインインに失敗しました', code: 'UNEXPECTED_ERROR' });
    }
  }
}
```
