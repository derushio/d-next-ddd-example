...
| 変数名 | 必須 | デフォルト | 説明 |
|--------|------|------------|------|
| `NEXT_PUBLIC_BASE_URL` | - | （空欄 = portless管理） | アプリケーションのベースURL |

開発環境では portless がURL管理を行うため、`NEXT_PUBLIC_BASE_URL` は空欄のままにしてください。
起動中のURLは `portless list` コマンドで確認できます。

```env
# 開発環境（portless管理のため空欄）
...
| 変数名 | 必須 | デフォルト | 説明 |
|--------|------|------------|------|
| `AUTH_URL` | ✅ | - | Auth.js v5 アプリケーションURL（`NEXTAUTH_URL` の後継） |
| `AUTH_SECRET` | ✅ | - | Auth.js v5 署名・暗号化シークレット（32文字以上推奨） |
...
AUTH_SECRET="your-auth-secret-key-minimum-32-chars"
...
- パスワードハッシュ: Argon2id（memoryCost:19456, timeCost:2, parallelism:1）を使用（環境変数不要、コードに定数定義）
...
AUTH_SECRET="your-very-long-random-secure-auth-secret-minimum-64-characters-recommended"
...
- **環境変数定義**: `src/lib/env.ts`（`@t3-oss/env-nextjs` の `createEnv` パターン — 詳細は `env-management` スキル参照）
