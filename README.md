...
1. **[Clean Architecture とは](./_DOCS/guides/ddd/concepts/clean-architecture.md)** - 設計思想と従来構成との違い
2. **[レイヤー構造の全体像](./_DOCS/architecture/layers/overview.md)** - 各レイヤーの責務と依存方向
3. **[最初の機能を実装する](./_DOCS/guides/development/first-feature.md)** - 手を動かしながら機能を実装
4. **[よくある問題とトラブルシューティング](./_DOCS/troubleshooting/common-issues.md)** - つまづきポイントの解決
...
- **[プロジェクト設計判断](./_DOCS/guides/implementation/decision-guide.md)** - アーキテクチャの意思決定記録
...
> **`_DOCS/` はテンプレート配布物のため読み取り専用です。**
> 編集してもテンプレート再適用（`sup-next`）時に上書き消失します。
> このプロジェクト固有のドキュメントは `_DOCS/` の外（この README や独自ディレクトリ）に書いてください。
> `_DOCS/` 自体の修正はテンプレ配布リポジトリ `d-next-template-debug` 側で行います。
...
- [ユニットテスト](./_DOCS/guides/testing-with-clean-architecture.md) - Clean Architectureにおけるテスト実装
