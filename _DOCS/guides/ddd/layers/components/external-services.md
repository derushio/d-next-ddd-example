...
export class ExternalApiService implements IExternalApiService {
 ...
 async validateAddress(address: string): Promise<AddressValidationResult> {
  ...
  try {
   ...
   this.logger.info('住所検証API呼び出し完了', {
    address,
 ...
 async getExchangeRate(
  ...
  try {
   ...
   this.logger.info('為替レートAPI呼び出し完了', {
    fromCurrency,
    toCurrency,
    rate,
   });
   ...
   this.logger.error('為替レートAPI呼び出し失敗', {
    fromCurrency,
    toCurrency,
    error,
   });
...
export class UnifiedEmailServiceAdapter implements IEmailService {
 ...
 async sendWelcomeEmail(email: string, name: string): Promise<void> {
  try {
   ...
   this.logger.warn('プライマリサービス失敗、フォールバックに切り替え', {
    error,
   });
...
export class ResilientExternalService {
 ...
 async validateAddressWithFallback(
  address: string,
...
describe('SendGridEmailService', () => {
 ...
 describe('sendWelcomeEmail', () => {
  it('正常にウェルカムメールが送信されること', async () => {
   ...
   expect(mockLogger.info).toHaveBeenCalledWith('ウェルカムメール送信完了', {
  ...
  it('送信失敗時にExternalServiceErrorが投げられること', async () => {
   ...
   expect(mockLogger.error).toHaveBeenCalledWith('ウェルカムメール送信失敗', {
    email: 'user@example.com',
    error: sendError,
   });
  });
