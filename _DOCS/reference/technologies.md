...
- **主要機能**: ユーティリティファースト、レスポンシブ、OKLCH色空間対応（広色域・知覚均一性）
...
- **依存**: radix-ui統一パッケージ（`radix-ui` 単一パッケージ、旧 `@radix-ui/react-*` 個別パッケージから移行済み）
...
- **バージョン**: 5.0.0-beta.30（next-auth v5）
- **選択理由**: Next.js App Router対応、Server Actions統合、Edge Runtime対応
- **主要機能**: セッション管理、JWTトークン、OAuth Provider対応
- **設定**: `src/auth.ts`、`AUTH_URL`・`AUTH_SECRET` 環境変数
- **公式**: <https://authjs.dev/>

- **JWT**: トークンベース認証（Auth.js JWTセッション）
- **OAuth**: 外部サービス連携（Provider追加可能）
