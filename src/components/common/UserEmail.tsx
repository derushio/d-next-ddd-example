...
export async function UserEmail() {
  try {
    ...
    return (
      <div
        className={clsx(
          'flex items-center space-x-2 px-3 py-1.5 rounded-lg bg-red-500/20 backdrop-blur-sm',
        )}
      >
        <HiExclamationTriangle className={clsx('h-4 w-4 text-red-300')} />
        <span className={clsx('text-sm text-red-300 font-medium')}>
          取得エラー
        </span>
      </div>
    );
