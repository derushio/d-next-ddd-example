...
import { ok, err, Result } from '@/layers/application/types/Result';
...
export class CreateOrderUseCase {
  ...
  ): Promise<Result<CreateOrderResponse, AppError>> {
    try {
      ...
      return err({ message: 'エラーメッセージ', code: 'ERROR_CODE' });
    }
  }
}
```
...
import type { ITransaction } from '@/layers/infrastructure/persistence/prisma/ITransaction';
...
export class PrismaOrderRepository implements IOrderRepository {
  ...
  private getClient(tx?: ITransaction): PrismaClient {
    return (tx as PrismaClient | undefined) ?? this.prisma;
  }

  async findById(id: OrderId, tx?: ITransaction): Promise<Order | null> {
    const client = this.getClient(tx);
    const prismaOrder = await client.order.findUnique({
      where: { id: id.value },
    });
    return prismaOrder ? this.toDomainObject(prismaOrder) : null;
  }

  async findAll(tx?: ITransaction): Promise<Order[]> {
    const client = this.getClient(tx);
    const prismaOrders = await client.order.findMany();
    return prismaOrders.map((o) => this.toDomainObject(o));
  }

  async save(order: Order, tx?: ITransaction): Promise<void> {
    const client = this.getClient(tx);
    await client.order.create({
      data: this.toPersistenceObject(order),
    });
  }

  async update(order: Order, tx?: ITransaction): Promise<void> {
    const client = this.getClient(tx);
  ...
  async delete(id: OrderId, tx?: ITransaction): Promise<void> {
    const client = this.getClient(tx);
    const existing = await client.order.findUnique({ where: { id: id.value } });
    if (!existing) {
      throw new DomainError('ORDER_NOT_FOUND', '注文が見つかりません');
    }
  ...
  private toDomainObject(prismaOrder: PrismaOrder): Order {
    return new Order(
  ...
  private toPersistenceObject(order: Order): Omit<PrismaOrder, 'createdAt' | 'updatedAt'> {
    return {
      id: order.id.value,
      // TODO: 他のプロパティをマッピング
    };
...
<% } -%>
  ...
  async execute(request: <%= name %>Request): Promise<Result<<%= name %>Response, AppError>> {
    // TODO: ビジネスロジックを実装してください
    return ok({
      // TODO: レスポンスを返却
    });
...
export class CreateOrderUseCase {
  ...
  async execute(request: CreateOrderRequest): Promise<Result<CreateOrderResponse, AppError>> {
    try {
      // 合計金額を計算
      ...
      return err({ message: '注文の作成に失敗しました', code: 'CREATE_ORDER_FAILED' });
    }
  }
}
```
...
3. **Prismaクライアントが古い**
   ...
   pnpm db:generate
   ```
   ※ `pnpm db:generate` は内部で `prisma generate --sql` が実行されます（TypedSQL対応）
