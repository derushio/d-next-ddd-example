...
export class SendGridEmailService implements IEmailService {
  ...
  async sendWelcomeEmail(email: string, name: string): Promise<void> {
    try {
      ...
      this.logger.error('ウェルカムメール送信エラー', { err: error, email });
  ...
  async sendPasswordResetEmail(email: string, resetToken: string): Promise<void> {
    ...
    try {
      ...
      this.logger.error('パスワードリセットメール送信エラー', { err: error, email });
...
export class S3StorageService implements IStorageService {
  ...
  async uploadFile(
    ...
    try {
      ...
      this.logger.error('S3ファイルアップロードエラー', { err: error, key });
  ...
  async getSignedDownloadUrl(key: string, expiresIn: number = 3600): Promise<string> {
    try {
      ...
      this.logger.error('S3署名付きURL生成エラー', { err: error, key });
  ...
  async deleteFile(key: string): Promise<void> {
    try {
      ...
      this.logger.error('S3ファイル削除エラー', { err: error, key });
...
export class LocalStorageService implements IStorageService {
  ...
  async uploadFile(
    ...
    try {
      ...
      this.logger.error('ローカルファイル保存エラー', { err: error, key });
  ...
  async deleteFile(key: string): Promise<void> {
    try {
      ...
      this.logger.error('ローカルファイル削除エラー', { err: error, key });
      throw new Error('ファイルの削除に失敗しました');
    }
  }
}
...
export class ExternalApiService implements IExternalApiService {
  ...
  private setupInterceptors(): void {
    ...
    this.client.interceptors.request.use(
      (config) => {
        this.logger.debug('外部APIリクエスト', { method: config.method?.toUpperCase(), url: config.url });
        return config;
      },
      (error) => {
        this.logger.error('外部APIリクエストエラー', { err: error });
    ...
    this.client.interceptors.response.use(
      (response) => {
        this.logger.debug('外部APIレスポンス', { status: response.status });
        return response;
      },
      (error) => {
        this.logger.error('外部APIレスポンスエラー', { err: error, status: error.response?.status });
        return Promise.reject(error);
      },
    );
  }
  ...
  private async retryRequest<T>(
    ...
    try {
      ...
      if (retries > 0 && this.isRetryableError(error)) {
        this.logger.warn('外部APIリトライ', { retriesLeft: retries });
...
// safeRegister: 二重登録を防ぐラッパー
function safeRegister<T>(
  childContainer: DependencyContainer,
  token: symbol,
  impl: new (...args: any[]) => T,
) {
  if (!childContainer.isRegistered(token)) {
    childContainer.registerSingleton(token, impl);
  }
}

// チャイルドコンテナ構造: core → domain → infrastructure → application
const infrastructureContainer = domainContainer.createChildContainer();

// 環境変数に基づいて実装を切り替え
if (env.NODE_ENV === 'production') {
  // 本番環境: 実際の外部サービスを使用
  safeRegister(infrastructureContainer, INJECTION_TOKENS.EmailService, SendGridEmailService);
  safeRegister(infrastructureContainer, INJECTION_TOKENS.StorageService, S3StorageService);
} else if (env.NODE_ENV === 'test') {
  // テスト環境: モック実装を使用
  safeRegister(infrastructureContainer, INJECTION_TOKENS.EmailService, MockEmailService);
  safeRegister(infrastructureContainer, INJECTION_TOKENS.StorageService, MockStorageService);
} else {
  // 開発環境: ローカル実装を使用
  safeRegister(infrastructureContainer, INJECTION_TOKENS.EmailService, MockEmailService);
  safeRegister(infrastructureContainer, INJECTION_TOKENS.StorageService, LocalStorageService);
}
```
...
export abstract class BaseExternalService {
  constructor(
    @inject(INJECTION_TOKENS.Logger) protected readonly logger: ILogger,
  ) {}

  protected handleError(error: unknown, context: string): never {
    this.logger.error(`${context}でエラーが発生しました`, { err: error, context });
